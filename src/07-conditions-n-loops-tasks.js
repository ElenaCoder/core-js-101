...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return a < b ? `${startChar}${a}, ${b}${endChar}` : `${startChar}${b}, ${a}${endChar}`;
}
...
function isCreditCardNumber(ccn) {
  const strFromNumber = String(ccn);
  let oddSum = 0;
  let doubleEvenSum = 0;
  for (let i = strFromNumber.length - 1; i >= 0; i -= 2) {
    oddSum += +strFromNumber[i];
  }
  for (let i = strFromNumber.length - 2; i >= 0; i -= 2) {
    let temp = 2 * +strFromNumber[i];
    if (temp >= 10) {
      temp = +(String(temp)[0]) + +(String(temp)[1]);
      doubleEvenSum += temp;
    } else {
      doubleEvenSum += 2 * +strFromNumber[i];
    }
  }
...
function getDigitalRoot(num) {
  ...
  number = sum;
  return sum < 10 ? number : getDigitalRoot(number);
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.has(str[i])) {
      stack.push(str[i]);
    } else if (closedBrackets.has(str[i])) {
